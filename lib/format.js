...
format.entries = function (entries) {
  const loc = format.errLoc(entries[0]);
  const frame = codeFrame(entries[0]);
  const trace = format.trace(_.tail(entries));
...
format.errLoc = function (entry) {
  return format.entry(entry, '@');
};

format.trace = function (entries=[]) {
  return _.map(entries, (entry) => format.entry(entry, figures.arrowRight)).join('\n');
};
